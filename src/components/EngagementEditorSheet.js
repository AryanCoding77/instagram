...
export default function EngagementEditorSheet({ visible, onClose }) {
  const { state, dispatch } = useReelData();
  const scrollRef = useRef(null);

  const [editorData, setEditorData] = useState(() => [...state.engagementPoints]);
  ...
  const getStepLabel = () => {
    if (stepSize === 0.5) return "+0.5%";
    if (stepSize === 1) return "+1%";
    if (stepSize === 2) return "+2%";
    return `+${stepSize}%`;
  };

  const cycleStepSize = () => {
    if (stepSize === 0.5) setStepSize(1);
    else if (stepSize === 1) setStepSize(2);
    else setStepSize(0.5);
  };

  // Update point value
  const updatePointValue = (index, raw) => {
    const value = Math.max(0, Math.min(20, parseFloat(raw.toFixed(1))));
    setEditorData((prev) => {
      const next = [...prev];
      next[index] = value;
      return next;
  ...
  // Handlers
  const handleReset = () => {
    setEditorData([...DEFAULT_DATA.engagementPoints]);
    setVideoDuration(DEFAULT_DATA.videoDuration);
    setSelectedPointIndex(0);
  };

  const handleDone = () => {
    dispatch({ type: "UPDATE_ENGAGEMENT", data: editorData, duration: videoDuration });
    onClose();
  };
        ...
        <View style={styles.modalContainer}>
          ...
          <View style={styles.container}>
            ...
            <View style={styles.header}>
              ...
              <Text style={styles.title}>When people liked</Text>
...
// Engagement Chart Preview Component
function EngagementChartPreview({ data, duration, selectedPointIndex, onPointSelect }) {
  ...
  const normalizeValue = (val) => val / 20; // Normalize to 0-1 range (max is 20%)

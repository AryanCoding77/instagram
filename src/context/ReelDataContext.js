  ...
  switch (action.type) {
    ...
    case "UPDATE_FIELD":
      ...
      if (action.field === "accountsReached" || 
        ...
        newState.rates = state.rates.map(rate => {
          switch (rate.key) {
            case "skip":
    ...
    case "UPDATE_TOP_SOURCE":
      return {
        ...state,
        topSources: state.topSources.map((s, i) =>
    ...
    case "UPDATE_AGE_GROUP":
      return {
        ...state,
        ageGroups: state.ageGroups.map((a, i) =>
          i === action.index 
            ? { 
    ...
    case "ADD_AGE_GROUP":
      return {
        ...state,
        ageGroups: [...state.ageGroups, { label: "New age", value: 0 }],
      };

    case "DELETE_AGE_GROUP":
      return {
        ...state,
        ageGroups: state.ageGroups.filter((_, i) => i !== action.index),
      };
    ...
    case "ADD_COUNTRY":
      return {
        ...state,
        countries: [...state.countries, { name: "New country", value: 0 }],
      };

    case "DELETE_COUNTRY":
      return {
        ...state,
        countries: state.countries.filter((_, i) => i !== action.index),
      };
